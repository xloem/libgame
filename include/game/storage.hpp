...
void storage_process(std::vector<uint8_t> & data, identifiers & what);
...
public:
	enum class process_result {
		...
		// if an unavoidable error is encountered, throw a process_error.  means processing cannot be fully completed.
	};
	storage();
	~storage();
	virtual process_result process(std::vector<uint8_t> & data, identifiers & what) = 0;
};
